...
def curvature_analysis(filename, y_label_name, output_file_name):
    ...
    # 傾きと切片を取得
    slope_tt = (df['TT'][12] - df['TT'][0]) / (df['temp'].max() - df['temp'].min())
    slope_ss = (df['SS'][12] - df['SS'][0]) / (df['temp'].max() - df['temp'].min())
    slope_ff = (df['FF'][12] - df['FF'][0]) / (df['temp'].max() - df['temp'].min())
    slope_fs = (df['FS'][12] - df['FS'][0]) / (df['temp'].max() - df['temp'].min())
    slope_sf = (df['SF'][12] - df['SF'][0]) / (df['temp'].max() - df['temp'].min())
...
def main():
    # ファイルパスを手動で入力する
    filename = input("曲率を求めたいCSVファイルのパスを入力してください: ")
