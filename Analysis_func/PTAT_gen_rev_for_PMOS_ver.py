...
def PTAT_csv_gen(VREF, VCTAT, output_file_csv, cor_val):
    ...
    df_VPTAT[columns_to_calculate] = round(df_VREF[columns_to_calculate] * cor_val - df_VCTAT[columns_to_calculate], 4)
...
def calculate_cor_val(VREF, VCTAT):
    ...
    # 40℃のデータを抽出
    df_40C_VCTAT = df_VCTAT[df_VCTAT['temp'] == 40].copy()
    df_40C_VREF = df_VREF[df_VREF['temp'] == 40].copy()
    ...
    df_40C_VCTAT['FF-SF'] = round(df_40C_VCTAT['FF'] - df_40C_VCTAT['SF'], 4)
    df_40C_VCTAT['SF-TT'] = round(df_40C_VCTAT['SF'] - df_40C_VCTAT['TT'], 4)
    df_40C_VCTAT['TT-FS'] = round(df_40C_VCTAT['TT'] - df_40C_VCTAT['FS'], 4)
    df_40C_VCTAT['FS-SS'] = round(df_40C_VCTAT['FS'] - df_40C_VCTAT['SS'], 4)
    # SS - SF, SF - TT, TT - FS, FS - FFを計算
    df_40C_VREF['FF-SF'] = round(df_40C_VREF['FF'] - df_40C_VREF['SF'], 4)
    df_40C_VREF['SF-TT'] = round(df_40C_VREF['SF'] - df_40C_VREF['TT'], 4)
    df_40C_VREF['TT-FS'] = round(df_40C_VREF['TT'] - df_40C_VREF['FS'], 4)
    df_40C_VREF['FS-SS'] = round(df_40C_VREF['FS'] - df_40C_VREF['SS'], 4)

    # 計算した列のみを新しいデータフレームに保存
    df_corner_diff_VCTAT = df_40C_VCTAT[['temp', 'FF-SF', 'SF-TT', 'TT-FS', 'FS-SS']]
    df_corner_diff_VREF = df_40C_VREF[['temp', 'FF-SF', 'SF-TT', 'TT-FS', 'FS-SS']]

    # 計算を行う列を指定（temp列以外）
    columns_to_calculate = ['FF-SF', 'SF-TT', 'TT-FS', 'FS-SS']
    df_cor_val = df_corner_diff_VCTAT[columns_to_calculate] / df_corner_diff_VREF[columns_to_calculate]

    # 各列の平均値を計算
    cor_val = round((df_cor_val['FF-SF'] + df_cor_val['SF-TT'] + df_cor_val['TT-FS'] + df_cor_val['FS-SS']) / 4, 2 )
    print(df_cor_val)
    print(df_40C_VREF)
    print(df_40C_VCTAT)
    return cor_val
...
def main():
    # ファイルパスを手動で入力する
    VREF = input("ソート後のVPTATのCSVファイルのパスを入力してください: ")
    VCTAT = input("ソート後のVREFのCSVファイルのパスを入力してください: ")
